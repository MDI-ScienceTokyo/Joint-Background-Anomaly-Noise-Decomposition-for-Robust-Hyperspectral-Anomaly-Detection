function results = JBAND_SSTV(HSI, params)
% 
% SSTV Version
%
%    min_{B,A,S,L} ||B||_{SSTV} + \lambda_1 ||A||_{2,1} + \lambda_2 ||L||_{1}
%    s.t. D_v(L) = 0
...
disp('****************** Algorithm starts (Proposed_SSTV) ******************')
...
if (mode == 1)
    for i = 1:max_iter
        ...
        % Updating B : Step 3
        B = B_pre - gamma_B.*(Dbt(Dvht(Y1_pre)) + Y3_pre);
        ...
        Y1_tmp = Y1_pre + gamma_Y1.*Dvh(Db(2*B - B_pre));
...
disp('****************** Algorithm ends (Proposed_SSTV) ********************')
