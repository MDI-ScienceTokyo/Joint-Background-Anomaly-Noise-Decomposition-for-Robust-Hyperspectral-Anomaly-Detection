function results = JBAND_HSSTV(HSI, params)
% 
% HSSTV Version
%
%    min_{B,A,S,L} ||B||_{HSSTV} + \lambda_1 ||A||_{2,1} + \lambda_2 ||L||_{1}
%    s.t. D_v(L) = 0
...
Dh  = @(z) z(:, [2:end, end], :) - z;
Dht = @(z) cat(2, -z(:, 1, :), -z(:, 2:(end-1), :) + z(:, 1:(end-2), :), z(:, end-1, :));
Db  = @(z) z(:, :, [2:end, end]) - z;
Dbt = @(z) cat(3, -z(:, :, 1), -z(:, :, 2:end-1) + z(:, :, 1:end-2), z(:, :, end-1));
H   = @(z) cat(4, Dv(Db(z)), Dh(Db(z)), omega*Dv(z), omega*Dh(z));
Ht  = @(z) Dbt(Dvt(z(:,:,:,1))) + Dbt(Dht(z(:,:,:,2))) + omega*Dvt(z(:,:,:,3)) + omega*Dht(z(:,:,:,4));
...
if use_GPU == 1
    ...
    Y1 = gpuArray(single(zeros(n1, n2, n3, 4)));
...
disp('****************** Algorithm starts (Proposed_HSSTV) ******************')
...
if (mode == 1)
    for i = 1:max_iter
        ...
        % Updating B : Step 3
        B = B_pre - gamma_B.*(Ht(Y1_pre) + Y3_pre);
        ...
        Y1_tmp = Y1_pre + gamma_Y1.*H(2*B - B_pre);
        Y1 = Y1_tmp - gamma_Y1.*proxL1Norm(Y1_tmp./gamma_Y1, 1./gamma_Y1);
...
disp('****************** Algorithm ends (Proposed_HSSTV) ********************')
